...
def search_patient_view(request):
    # 1. Get all patients from database, sort by first name
    ...
    # This will hold the found patient
    result = None

    # This will hold the search input
    query = ''
    if request.method == 'POST':
        query = request.POST.get('query') # Get the search text
        names = [p.user.first_name.lower() for p in patient_list]  # Make a lowercase name list
        index = binary_search(names, query.lower())  # Run binary search
        if index != -1:
            result = patient_list[index]  # If found, get the patient object


    return render(request, 'hospital/search_patient.html', {'result': result, 'query': query})
...
def binary_search(data, target):
    ...
    while low <= high:
        ...
        if data[mid] == target:
            ...
            low = mid + 1 # Target found, return its index search to right
        else:
